...
export class SvcMasterDataService {
  ...
  public CurriculumsGet = (IsActive?: boolean): Observable<ISPListBaseModel[]> => {
  ...
  public GetCurriculumsByRequestStatusID = (TopicRequestStatus: number): Observable<ISPListBaseModel[]> => {
    return new Observable((observer: Observer<ISPListBaseModel[]>) => {
      observer.next(Curriculums);
      observer.complete();
    });
