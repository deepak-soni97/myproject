...
export class TopicContributePageComponent implements OnInit {
  ...
  getTopics = async (id: number) => {
    ...
    debugger;
    // await this._service.TopicGetById(id).subscribe((data: any) => {
  ...
  onSubmit(submittionType: string, updateWF: boolean) {
    ...
    //return this.validateAllFormFields(this.basicForm);
  ...
  onAttachmentUploadChange(e: any) {
    ...
    item.clear();
    e.map((row: any) => {
      item.push(this.fb.group({ isDownloadable: row.isDownloadable, id: row.id, fileItem: row }))
    });
