...
export class TopicContributePageComponent implements OnInit {
  ...
  conditionalMinutesRequired(formGroup: FormGroup) {
    // if (formGroup.value.myCheckbox) {
    //   return Validators.required(formGroup.get('hourList')) ? {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(routeParams => {
      ...
      for (let minute = 0; minute <= 55; minute += 5) {
        ...
        this.Minutes.push({ id: (minute < 10 ? `0${minute}` : minute.toString()), title: minute + (minute == 0 ? ' Minute' : ' Minute') },
        )
  ...
  getTopics = async (id: number) => {
    ...
    if (data) {
      ...
      if (this.currentItem.status == 'Back to Initiator for update') {
        ...
        this.svcWfManager.GetCompletedWFTaskByWFInstanceID(this.currentItem.wfInstanceId).subscribe((wfHistory: any) => {
  ...
  bannerRemove() {
    this.croppedImage = null//this.croppedImagePre;
    this.basicForm.patchValue({
      BannerFile: null,
    });
  ...
  onSubmit(submittionType: string, updateWF: boolean) {
    ...
    if (this.basicForm.value.Attachments.length == 0 && this.ExistingTopicAttachments.length == 0) {
      this.attachmentError = true;
      this.fileAttachmentError = 'Please upload attachments';
    } else {
      this.validateAllFormFields(this.basicForm);
      this.scrollToError();
  ...
  onCoverFileSelected(e: any) {
    ...
    if (e.target.files) {
      ...
      if (fileSize <= 5 && e.target.files[0].type.toLowerCase().indexOf("image") > -1) {
        ...
        this.bannerError = 'Image file with max file size 5mb allowed.';
      }
    }
  }
  ...
  onAttachmentUploadChange(e: any) {
    let totalSize = (e.reduce((x: any, y: any) => x + y.size, 0)) / (100 * 1024 * 1024);
    if (totalSize > 1) {
      this.attachmentError = true;
      this.fileAttachmentError = "Total size of all attachment should not be more then 100 MB";
    } else {
  ...
  onHourSelect(event: any) {
    this.basicForm.patchValue({ hourList: event })
  }
  onMinuteSelect(event: any) {
    this.basicForm.patchValue({ minuteList: event })
  }
  onTopicLevelSelect(event: any) {
    this.basicForm.patchValue({ TopicLevel: event })
  }
  ...
  scrollToError(): void {
    const firstElementWithError = document.querySelector('.custerror');
